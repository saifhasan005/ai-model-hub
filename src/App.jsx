  ...
  return (
    <>
      ...
      <div className='tabs gap-[20px] tabs-box justify-center bg-transparent'>
        ...
        <input type="radio" name='my_tabs_1' className='shadow-xl font-bold tab rounded-full w-40' aria-label='Cart' onClick={()=> setactiveTab('cart')}/>
      </div>
      { activeTab =="model" ? <Cards setselectedData={setselectedData} selectedData={selectedData} modelPromise={modelPromise}></Cards> : ""}
      {activeTab == "cart" ? <Cart selectedData={selectedData}></Cart>:""}
      <Footer></Footer>
    </>
  )
}
